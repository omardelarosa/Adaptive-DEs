...
# # DLL path
_JADE_SO_PATH = 'JADE.*.so'
# the absolute path to the C shared object library
_LIB_PATH = os.path.join(_MODULE_PATH, "..",
                         _BUILD_PATH, "**", _JADE_SO_PATH)

# package root dir
_LIB_PATH_ROOT = os.path.join(_MODULE_PATH, "..", _JADE_SO_PATH)

_JADE = None

# check build dir
try:
    _JADE = c.cdll.LoadLibrary(glob.glob(_LIB_PATH)[0])
except IndexError:
    # check root directory
    try:
        _JADE = c.cdll.LoadLibrary(glob.glob(_LIB_PATH_ROOT)[0])
    except IndexError:
        raise OSError('missing static JADE*.so library!')
